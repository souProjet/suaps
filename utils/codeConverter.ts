/**
 * Utilitaires pour la conversion des codes cartes SUAPS
 * Basé sur l'analyse du système SUAPS officiel
 */
 ...
 */
export function convertCodeCarteToHex(codeNumerique: string): string {
 ...
 */
export function convertHexToCodeCarte(codeHex: string): string {
 ...
 */
export function processCodeCarte(codeCarte: string): string {
  ...
  // Si c'est uniquement numérique, le convertir en hexa
  if (/^\d+$/.test(cleanCode)) {
    return convertCodeCarteToHex(cleanCode);
  }
  
  // Sinon retourner tel quel (normalisé en majuscules)
 ...
 */
export function validateCodeCarte(codeCarte: string): { isValid: boolean; message?: string } {
 ...
 */
export function formatCodeCarteForDisplay(codeCarte: string): string {
  const cleanCode = codeCarte.replace(/\s/g, '').trim();
 ...
 */
export function detectCodeCarteType(codeCarte: string): 'numeric' | 'hex' | 'unknown' {
  const cleanCode = codeCarte.replace(/\s/g, '').trim();
